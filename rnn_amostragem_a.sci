...
function [erro_ok]=checar_erro_rnn(hist_mse, erro_minimo)
    erro_ok = hist_mse($) < erro_minimo
endfunction

redes = list();

while length(redes) < 2
    [W,P_TESTE,T_TESTE,P_,T_,hist_mse]=treinar_rnn(dados_entrada, dados_esperados_saida)
    erro_ok = checar_erro_rnn(hist_mse, mse_min)
    if erro_ok then
        redes($+1) = W
    end
    close(winsid()) // FECHA TODAS AS JANELAS / FIGURAS
...
function saidas=executar_comite(lista_redes, dados_entrada)
    tamanho_lista_redes = length(lista_redes)
    tamanho_entrada = (length(dados_entrada) / 2) + 1 // duas linhas por coluna
    indice_redes = 1
    ...
    while indice_dados_entrada ~= tamanho_entrada 
        ...
        while indice_redes ~=  tamanho_lista_redes
            indice_dados = indice_dados_entrada
            media_do_comite_para_o_indice = ann_FFBP_run(dados_entrada(:,indice_dados),lista_redes(indice_redes),af) + media_do_comite_para_o_indice;
            indice_redes = indice_redes + 1
        end
        media_do_comite_para_o_indice = media_do_comite_para_o_indice / tamanho_lista_redes
        saidas ($ + 1) = media_do_comite_para_o_indice
        indice_dados_entrada = indice_dados_entrada +1
    end
endfunction

saidas_comite = executar_comite(redes, dados_entrada)
...
////testex = T_TESTE;
//teste = ann_FFBP_run(P_,W,af);
//testex = T_;

figure(103);
title("Entrada Original: Saída da Rede vs Saída Esperada")
saidas_comite_vetor = list2vec(saidas_comite)' // Transpor o vetor que veio da lista :))))

plot(saidas_comite_vetor,saidas_comite_vetor,'+r')
plot(dados_esperados_saida, saidas_comite_vetor, '.b');
legend('Saída da Rede', 'Saída Esperada')
